...
public class VideoOutputManager {
    ...
    public void create(long handle, TextureUpdateCallback textureUpdateCallback) {
        synchronized (lock) {
            Log.i(TAG, String.format(Locale.ENGLISH, "com.mohammed.flutter_mpv_video.VideoOutputManager.create: %d", handle));
    ...
    public void dispose(long handle) {
        synchronized (lock) {
            Log.i(TAG, String.format(Locale.ENGLISH, "com.mohammed.flutter_mpv_video.VideoOutputManager.dispose: %d", handle));
            if (videoOutputs.containsKey(handle)) {
    ...
    public void setSurfaceSize(long handle, int width, int height) {
        synchronized (lock) {
            Log.i(TAG, String.format(Locale.ENGLISH, "com.mohammed.flutter_mpv_video.VideoOutputManager.setSurfaceSize: %d %d %d", handle, width, height));
            if (videoOutputs.containsKey(handle)) {
